...
@Component({
  selector: 'ngx-orange-modals',
  templateUrl: './orange-modals.component.html',
  styleUrls: ['./orange-modals.component.scss']
})
export class OrangeModalsComponent implements OnInit {
  ...
  constructor(private excelExportService: ExcelExportService, private detaisSerivce: DetailsService, private httpClient: HttpClient,
  ...
  ngOnChanges() { // <- it will run every time and give you the latest value of fieldType
    console.log("66----->"+ this.Details.id);
    if (this.Details) {
      this.httpClient.get('http://localhost:8081/get/' + this.Details.id).subscribe(
        res => {
  ...
  ngOnInit(): void {
    ...
    console.log("------> 83 " + this.Details)
    // let routeId = this.ar.snapshot.paramMap.get('id');
    let id = this.Details;  /// car les param tj considerés comme String dans l'url
    console.log(id);
    if (!isEmptyObject(id)) {
