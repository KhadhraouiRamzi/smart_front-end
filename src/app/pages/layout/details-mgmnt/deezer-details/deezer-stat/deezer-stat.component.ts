...
@Component({
  selector: 'ngx-deezer-stat',
  templateUrl: './deezer-stat.component.html',
  styleUrls: ['./deezer-stat.component.scss']
})
export class DeezerStatComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.deezerSerivce.getStatArtisteDeezer().subscribe(
      res => {
        let role = this.token.getUser()['roles'];
        let idUser = this.token.getUser().id;

        if (role == "ROLE_ARTISTE") {
          this.deezerSerivce.getStatArtisteDeezerById(idUser).subscribe(data => {
            this.details = data;
