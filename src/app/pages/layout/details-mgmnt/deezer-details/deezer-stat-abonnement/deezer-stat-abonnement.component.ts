  ...
  @Component({
    selector: 'ngx-deezer-stat-abonnement',
    templateUrl: './deezer-stat-abonnement.component.html',
    styleUrls: ['./deezer-stat-abonnement.component.scss']
  })
  export class DeezerStatAbonnementComponent implements OnInit {
    ...
    constructor(private excelExportService: ExcelExportService,
      private deezerSerivce: DeezerService, private r: Router, public token: TokenStorageService) {
    }
    ...
    ngOnInit(): void {
      ...
      this.deezerSerivce.getStatAbonnementDeezer().subscribe(
        res => {
          let role = this.token.getUser()['roles'];
          let idUser = this.token.getUser().id;
          if (role == "ROLE_ARTISTE") {
            this.deezerSerivce.getStatAbonnementDeezerById(idUser).subscribe(data => {
              this.details = data;
              ...
              setTimeout(function () {
                (function ($) {
                  $(document).ready(function () {
                    $('#table-deezer-stat-abonnement').DataTable({
                      "footerCallback": function (row, data, start, end, display) {
                        ...
                        var totalPartArtiste = apiFiltre
                          ...
                          $(apiFiltre.column(1).footer()).html(formatNumberEspace((ttc/1000).toFixed(3))); 
                          $(apiFiltre.column(2).footer()).html(formatNumberEspace(nbr_ecoute.toFixed()));
                          $(apiFiltre.column(3).footer()).html(formatNumberEspace((part_smart/1000).toFixed(3)));
                          $(apiFiltre.column(4).footer()).html(formatNumberEspace((part_artiste/1000).toFixed(3)));
    ...
    exportexcel(): void {
      ...
      let element = document.getElementById('table-deezer-stat-abonnement');
    ...
    public openPDF(): void {
      let DATA = document.getElementById('table-deezer-stat-abonnement');
